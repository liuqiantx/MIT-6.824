...
- 限制了能让日志变得不一致的方式???
- 尽量消除非确定性.(使系统更易于理解)
- 随机虽然引入了不确定性,但它减少了需要记住的状态.(随便选吧,反正都一样.)

term 类似于逻辑时钟,让各个服务清楚过期的信息.
每一次 rpc 都会沟通 term.

一个统一的整体．少部分成员挂掉，团队依旧存活．　-> 大规模的软件系统.
方便开发人员理解，实操．
赋予领导更强的权利．（日志仅能由领导分发）．
领导选举的公平性．（随机时间，先知先得 -> 有资格的前提下）
新旧配置多数重叠以最大程度降低更改配置对系统运行的影响 -> 具体？

安全性? -> 状态机的安全
- 相同的**日志应用索引**不允许存在不同的日志
- 每一个任期只允许存在一个 leader.
- leader 本身的日志,只允许添加,不能重写或删除.
- 如果两个日志列表包含了相同的任期和日志索引,那么该索引前的所有日志,两个日志列表都一致.
- 如果一个日志在给定的任期被提交了,那么该日志在所有之后任期的领导者的日志中都会存在.(领导者的日志列表必须完整,对于所有已经提交的日志)
- 如果一个服务在给定的索引**应用**了一个日志,那么不可能存在其他服务在该索引**应用**了一个不一样的日志的情况
